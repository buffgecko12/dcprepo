...
-- School / class
CREATE OR REPLACE VIEW $APP_NAME$Views.SchoolAll AS SELECT * FROM $APP_NAME$.School; -- History rows
CREATE OR REPLACE VIEW $APP_NAME$Views.School AS SELECT * FROM $APP_NAME$Views.SchoolAll WHERE EndTS = TIMESTAMP '9999-12-31 23:59:59';
...
SELECT UserId, SchoolId, UserName, UserType, FirstName, LastName, CAST(FirstName || ' ' || LastName AS VARCHAR(250)) AS UserDisplayName, 
	   EmailAddress, Password, ReputationValue, ReputationValueLastSeenTS, ProfilePictureId, SharedAccountFlag,  
...
SELECT r.RewardId, r.SchoolYear, r.RewardDisplayName, r.RewardDescription, r.RewardValue, r.RewardCategory, rc.CategoryDisplayName AS RewardCategoryDisplayName, 
	   r.Vendor, r.SourceRewardId, r.StartTS, r.EndTS
